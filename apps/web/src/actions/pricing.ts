...
export async function syncPricing(formData: FormData) {
  const slug = formData.get('workspace') as string;
  const brandId = formData.get('brand_id') as string;
  const ws = await resolveWorkspace(slug);
  if (!ws) return;

  await aquilaFetch(ws.id, `/brands/${encodeURIComponent(brandId)}/pricing-sync`, {
    method: 'POST',
  });

  revalidatePath(`/${slug}/brands/${brandId}/pricing`);
}
