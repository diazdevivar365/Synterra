#!/usr/bin/env bash
# Deploy script for forgentic-cache.lan (192.168.10.51)
# Manages: Redis 7 + nightly RDB backup + exporters
#
# Deployed copy: /opt/deploy-cache.sh on forgentic-cache.lan
# To sync after editing:
#   sudo cp ~/Forgentic/Synterra/infra/lxc-cache/deploy-cache.sh /opt/deploy-cache.sh
#   sudo chmod +x /opt/deploy-cache.sh
#
# Bootstrap on a fresh LXC:
#   sudo mkdir -p /etc/forgentic
#   sudo tee /etc/forgentic/deploy.secret > /dev/null <<EOF
#   INFISICAL_CLIENT_ID=<cache-machine-identity-client-id>
#   INFISICAL_CLIENT_SECRET=<cache-machine-identity-client-secret>
...
COMPOSE_FILE="$INFRA_DIR/lxc-cache/docker-compose.yml"
ENV_FILE="$INFRA_DIR/lxc-cache/.env"
