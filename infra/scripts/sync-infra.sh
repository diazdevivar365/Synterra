...
#
# Usage:
#   bash infra/scripts/sync-infra.sh                    # sync all nodes
#   bash infra/scripts/sync-infra.sh app                # sync one node
#   bash infra/scripts/sync-infra.sh db cache metering  # sync multiple nodes
#
...
sync_db() {
  ...
  ssh forgentic@192.168.10.50 "mkdir -p /home/forgentic/infra/lxc-db"
  push "$INFRA/lxc-db/docker-compose.yml" 192.168.10.50 /home/forgentic/infra/lxc-db/docker-compose.yml
  push "$INFRA/lxc-db/deploy-db.sh"       192.168.10.50 /home/forgentic/bin/deploy-db.sh
  mark_runnable 192.168.10.50 /home/forgentic/bin/deploy-db.sh
}

sync_cache() {
  echo "→ cache (192.168.10.51)"
  ssh forgentic@192.168.10.51 "mkdir -p /home/forgentic/infra/lxc-cache"
  push "$INFRA/lxc-cache/docker-compose.yml" 192.168.10.51 /home/forgentic/infra/lxc-cache/docker-compose.yml
  push "$INFRA/lxc-cache/deploy-cache.sh"    192.168.10.51 /home/forgentic/bin/deploy-cache.sh
  mark_runnable 192.168.10.51 /home/forgentic/bin/deploy-cache.sh
}

sync_metering() {
  echo "→ metering (metering.lan)"
  ssh forgentic@metering.lan "mkdir -p /home/forgentic/infra/lxc-metering"
  push "$INFRA/lxc-metering/docker-compose.yml" metering.lan /home/forgentic/infra/lxc-metering/docker-compose.yml
  push "$INFRA/lxc-metering/deploy-metering.sh" metering.lan /home/forgentic/bin/deploy-metering.sh
  mark_runnable metering.lan /home/forgentic/bin/deploy-metering.sh
}

sync_observability() {
  echo "→ observability (observability.lan)"
  ssh forgentic@observability.lan "mkdir -p /home/forgentic/infra/lxc-observability/config"
  ...
  push "$INFRA/lxc-observability/deploy-observability.sh" observability.lan /home/forgentic/bin/deploy-observability.sh
  push_config_dir "$INFRA/lxc-observability/config"       observability.lan /home/forgentic/infra/lxc-observability/config
  mark_runnable observability.lan /home/forgentic/bin/deploy-observability.sh
}
...
echo ""
echo "✓ Sync complete. To apply: ssh <node> && bash /home/forgentic/bin/deploy-<node>.sh"
